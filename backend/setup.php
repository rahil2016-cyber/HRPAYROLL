...
try {
    ...
    if ($userCount == 0) {
        ...
        $attStmt = $db->prepare("INSERT INTO attendance (employee_id, date, clock_in, clock_out, clock_in_lat, clock_in_lng, status, is_wfh) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $attStmt->execute([$empEmpId, date('Y-m-d', strtotime('-1 day')), '09:05', '18:10', 12.9716, 77.5946, 'Present', 0]);
        $attStmt->execute([$empEmpId, date('Y-m-d', strtotime('-2 days')), '09:12', '18:05', 12.9715, 77.5947, 'Present', 0]);
